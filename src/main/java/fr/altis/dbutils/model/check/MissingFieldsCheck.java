...
public class MissingFieldsCheck implements Check
{
    private List<Table> referenceTables;

    private Map<String, Table> testedTablesMap;

    public MissingFieldsCheck(List<Table> referenceTables, List<Table> testedTables)
    {
    ...
    public List<Difference> check()
    {
        ...
        for (Table referenceTable : referenceTables)
        {
            ...
            {
                differences.add(new Difference(referenceTable.getName() + "/" + missingField, Check.MISSING_FIELDS));
            }
        }
